...
- [ ] 1.1 创建 Xcode 项目 `CXSwitch`，配置 Bundle ID `com.bigo.cx-switch`，最低部署目标 macOS 14.0
...
- [ ] 3.3 实现 `KeychainService`：使用 Security framework 的 `SecItemAdd` / `SecItemCopyMatching` / `SecItemDelete`，Service 为 `com.bigo.cx-switch.account`，Value 为 Base64(AuthBlob JSON)
...
- [ ] 6.3 实现 `CurrentAccountSection`：当前账户邮箱、plan badge、UsageBar × 2、Save 按钮
- [ ] 6.4 实现 `SavedAccountRow`：可点击账户行，含邮箱、plan badge、primary 用量条
- [ ] 6.5 实现 `MenuBarView`：组装 CurrentAccountSection + 已保存账户列表 + Add/Refresh + 错误提示 + Footer
- [ ] 6.6 实现 `LoginFlowSheet`：登录状态提示（starting/waiting/completed/error）
- [ ] 6.7 实现 `FooterActions`：Settings、OpenAI Status、Quit 按钮
- [ ] 6.8 实现 Refresh Token 导入入口：在 Add Account 区域或菜单中提供 "Import Refresh Token" 操作，支持粘贴 token 并触发导入流程
